...
foreach ($order->getBasket() as $basketItem) {
    ...
    if ($ob = $res->GetNextElement()) {
        ...
        $arBasketItem['CML2_ARTICLE'] = $arProps['CML2_ARTICLE']['VALUE'];
    }
    ...
    if($arBasketItem['BASE_PRICE'] == $arBasketItem['PRICE']) {
        unset($arBasketItem['BASE_PRICE']);
        unset($arBasketItem['BASE_PRICE_DISPLAY']);
    }
...
if($arBasketItem['SUM_BASE'] == $arBasketItem['SUM']) {
    unset($arBasketItem['SUM_BASE']);
    unset($arBasketItem['SUM_BASE_DISPLAY']);
}

...
BX.ready(function ()
{
    ...
    BX.bindDelegate(
        ...
        function (e)
        {
            ...
            data = new FormData(loadFileForm);

            // files.forEach(file => data.append('inputFiles[]', file));

            let submitSend = this
            submitSend.classList.add('btn_status_loading')
            submitSend.setAttribute('disabled', 'disabled');
            errorBox.style.display = 'none';
            BX.ajax({
                url: '/local/ajax/send_file_contact.php',  //адрес на который передаются данные с формы
                ...
                {  //в случаи успеха, выполняем действия
                    ...
                    submitSend.removeAttribute('disabled');
                    if(resultRequest.result == 'error'){
                        errorBox.style.display = 'block';
                        errorBox.textContent = resultRequest.texterror
                    }else {

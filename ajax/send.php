...
function sendLeadEmail($data) {
	...
	$subject = "Лид с сайта TINGERPLAST.RU";
...
$data['UF_CRM_1449663065'] = $_POST['direction'] !== '' ? $_POST['direction'] : 14016;
$data['SOURCE_ID'] = 'CONFERENCE';
...
if (isset($_POST['cartItems'])) {
	...
	$data['COMMENTS'] .= '<b>Состав заказа:</b> <br>';
	foreach ($cartItems as $item) {
		$name = $item['NAME'];
		$quantity = $item['QUANTITY'];
		$price = $item['SUM_FULL_PRICE_FORMATED'];
		$data['COMMENTS'] .= $name . ' (' . $quantity . ' шт.), на сумму ' . $price . '<br>';
	}
	$data['COMMENTS'] .= 'Общая сумма заказа: <b>' . $_POST['cartSum'] . '</b>';
}

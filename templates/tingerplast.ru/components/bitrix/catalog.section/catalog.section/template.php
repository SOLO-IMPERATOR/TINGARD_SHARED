...
/* global $USER; if ($USER->IsAdmin()) {
	...
	if($arResult['ID'] == '105' && $arResult['DEPTH_LEVEL'] == 1) {
		echo 'okk';
	}
...
/* 	if (($arParams['HIDE_SECTION_DESCRIPTION'] !== 'Y') && !empty($arResult['DESCRIPTION']))
	...
	?>
		<div class="ajax">
		<div class="preview-product__list ajax-container" itemscope itemtype="http://schema.org/ItemList">
			<!-- items-container -->
			<?
		...
		<?if($showLazyLoad):?>
			<div class="text-center mb-4" data-entity="lazy-<?=$containerName?>">
				<button type="button" data-use="show-more-<?=$navParams['NavNum']?>"><?=$arParams['MESS_BTN_LAZY_LOAD']?></button>
			</div>
		<?endif?>

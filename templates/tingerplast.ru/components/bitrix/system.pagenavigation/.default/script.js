const ajaxContainers = document.querySelectorAll('.ajax');
if (ajaxContainers.length > 0) {
	ajaxContainers.forEach((ajaxContainer) => {
		let ajaxBtnContainer = ajaxContainer.querySelector('.ajax-btn-container');
		if (ajaxBtnContainer) {
			ajaxBtnContainer.addEventListener('click', function(event) {
				...
				if (event.target.closest('.ajax-btn')) {
					...
					const targetContainer = ajaxContainer.querySelector('.ajax-container');

						...
						<label class="conf-widget__options-item conf-widget__options-item_type_with-img" data-modul-binding="" data-sim-code="<?= $put_dopmodules["UF_CODE"] ?>" data-id-modul="<?= $put_dopmodules['UF_XML_ID'] ?>" data-mods-name="<?= $mods_name ?>" data-name="<?= $put_dopmodules['UF_NAME'] ?>" data-price="<?= $put_dopmodules['UF_PRICE'] ?>" <? if ($put_dopmodules["UF_BLOCKED"] == 1 && $put_dopmodules["UF_CHECKED"] == 1) echo "data-default='default'"; ?>>
							...
							<span class="checkbox__container">
								<span class="checkbox__img-container">
									...
									<?else:?>
									<img class="checkbox__img" src="<?=SITE_TEMPLATE_PATH?>/img/no-img.svg" alt="<?=$put_dopmodules['UF_NAME']?>">
									<?endif?>
								</span>

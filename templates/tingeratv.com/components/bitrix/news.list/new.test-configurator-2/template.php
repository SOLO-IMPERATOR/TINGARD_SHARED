...
<section data-page="first-page" class="container margin_b_xl conf-preloader conf-preloader_active">
	<nav class="conf-nav margin_b_l">
		<ul class="conf-nav__list">
			<li class="conf-nav__item conf-nav__item_status_active">
				<a class="conf-nav__link" href="#"><span class="conf-nav__step">1</span> <span class="conf-nav__text">Model selection</span></a>
			</li>
			<li class="conf-nav__item">
				<a class="conf-nav__link" href="#"><span class="conf-nav__step">2</span> <span class="conf-nav__text">Modification selection</span></a>
			</li>
			<li class="conf-nav__item">
				<a class="conf-nav__link" href="#"><span class="conf-nav__step">3</span> <span class="conf-nav__text">Equipment selection</span></a>
			</li>
			<li class="conf-nav__item">
				<a class="conf-nav__link" href="#"><span class="conf-nav__step">4</span> <span class="conf-nav__text">Terms of purchase</span></a>
			</li>
		</ul>
	</nav>
	<h2 class="section-title margin_b_m">Select model</h2>
	<div class="conf-models margin_b_m">
		<ul class="conf-models__list">
			<?foreach ($arResult['ITEMS'] as $pet_productions):?>
			<li class="conf-models__item">
				...
				<input class="radio__input" type="radio" name="models" data-code-production="<?= $pet_productions["CODE"] ?>">
					<span class="radiobox__container">
						...
						<span class="radiobox__text">choose</span>
					</span>
	...
	<button id="continue-btn-2" class="btn btn_color_green w--100">Continue</button>
</section>

<? foreach ($arResult['ITEMS'] as $pet_model_data) { ?>
	<?
		...
		while ($arData = $rsData_modifications->Fetch()) {
			foreach ($pet_model_data["DISPLAY_PROPERTIES"]["HL_MODS"]["VALUE"] as $get_modifications) {
				if ($get_modifications == $arData["UF_XML_ID"]) {
					$pet_model_data["MODIFICATIONS"][$pet_model_data["ID"]][] = [$arData["UF_NAME"], $arData["ID"], CFile::GetPath($arData["UF_IMG"]), $arData["UF_NCREASEL"], $arData['UF_MODIFICATION_CHARS']];
				}
			}
		}
	}
	...
	<section data-page="equipment" data-simbol-code="<?= $pet_model_data['CODE'] ?>" style="display:none;" class="container margin_b_xl">
		...
		<h2 class="section-title margin_b_m">Select modification</h2>
			...
			<div class="conf-modification__visual">
				<div class="conf-modification__choice">
					<label class="conf-modification__choice-item">
						<span class="section-subtitle margin_b_s">Modification and engine</span>
						<select data-simbol-code="<?= $pet_model_data['CODE'] ?>" class="select" name="modification">
							<? for ($i = 0; $i < count($pet_model_data["MODIFICATIONS"][$pet_model_data["ID"]]); $i++) { ?>
								<option data-mods-name="<?= $pet_model_data["MODIFICATIONS"][$pet_model_data["ID"]][$i][0] ?>" data-mods-img="<?= $pet_model_data["MODIFICATIONS"][$pet_model_data["ID"]][$i][2] ?>" value="<?= $pet_model_data["MODIFICATIONS"][$pet_model_data["ID"]][$i][3] ?>"><?= $pet_model_data["MODIFICATIONS"][$pet_model_data["ID"]][$i][0] ?></option>
							<? } ?>
						</select>
					</label>
				</div>
				<div class="conf-modification__layers conf-layers">
					<img class="conf-layers__img-bg" src="/local/templates/tinger/img/conf/transparent-bg.png" alt="Transparent background">

					<? foreach ($pet_model_data["MODIFICATIONS"][$pet_model_data["ID"]] as $set_mods_img) { ?>
						<img class="conf-layers__img conf-layers__img--base tr5-tank" data-mods-name="<?= $set_mods_img[0] ?>" src="<?= $set_mods_img[2] ?>" alt="<?= $set_mods_img[0] ?>">
					<? } ?>

				</div>
			</div>
			<div class="conf-modification__chars">
				<span class="section-subtitle margin_b_s">Characteristics</span>
				<table class="table margin_b_s">
					<?foreach($pet_model_data["MODIFICATIONS"][$pet_model_data["ID"]] as $charsItems):?>
						<?foreach($charsItems[4] as $charsItem):?>
		...
		<div class="btn__group">
			<button data-btn-specification="back-btn-1" class="btn btn_color_green">Black</button>
			<button data-btn-specification="continue-btn-3" data-simbol-code="<?= $pet_model_data['CODE'] ?>" class="btn btn_color_green">Continue</button>
		</div>
	</section>
<? } ?>

<? foreach ($arResult['ITEMS'] as $get_model) { ?>
	...
	<section data-page="conditions" data-simbol-code="<?= $get_model["CODE"] ?>" data-mods-name="" style="display:none;" class="container margin_b_xl">
		<nav class="conf-nav margin_b_l">
			<ul class="conf-nav__list">
				<li class="conf-nav__item conf-nav__item_status_success" data-block-id="first-page" data-model-name="">
					<a class="conf-nav__link" href="#"><span class="conf-nav__step">1</span> <span class="conf-nav__text">Model selection</span></a>
		...
		<h2 class="section-title margin_b_m">Select equipment</h2>
		...
		<div class="conf-widget margin_b_m">
			...
			<? if (is_countable($get_model["PACKS"][$get_model["ID"]])) { ?>
					...
					<div class="conf-widget__packages margin_b_s">
						...
						?>
							...
							<label class="conf-widget__package" data-modul-name="<?= $active_modification ?>" data-modul-collection="<?= $modul_collection ?>" data-simbol-code="<?= $get_model["CODE"] ?>">
								<input class="conf-widget__package-input" data-mods-name="" data-modul-collection="<?= $modul_collection ?>" data-simbol-code="<?= $get_model["CODE"] ?>" type="radio" onMouseDown="this.isChecked=this.checked;" onClick="this.checked=!this.isChecked;" name="package" value="0">
				...
				<div class="conf-widget__slider margin_b_m">
						...
						<div class="swiper-wrapper">
							<div class="swiper-slide conf-widget__slide">
								<div class="conf-layers" data-side-img="layer-front">
									...
									<?if(count($get_model['DISPLAY_PROPERTIES']['CONF_COLORED_BODY_FRONT']['DESCRIPTION']) === 1):?>
										<img class="conf-layers__img" src="<?= $get_model['DISPLAY_PROPERTIES']['CONF_COLORED_BODY_FRONT']['FILE_VALUE']['SRC'] ?>" data-color-body="<?= $get_model['DISPLAY_PROPERTIES']['CONF_COLORED_BODY_FRONT']['DESCRIPTION'][0] ?>" data-name="color" alt="<?= $get_model['DISPLAY_PROPERTIES']['CONF_COLORED_BODY_FRONT']['DESCRIPTION'][0] ?>">
									<?else:?>
							...
							<? if (!empty($get_model['DISPLAY_PROPERTIES']['CONF_COLORED_BODY_BACK']['FILE_VALUE'])) { ?>
								<div class="swiper-slide conf-widget__slide">
									<div class="conf-layers" data-side-img="layer-back">
										...
										<?if(count($get_model['DISPLAY_PROPERTIES']['CONF_COLORED_BODY_BACK']['DESCRIPTION']) === 1):?>
										<img class="conf-layers__img" src="<?= $get_model['DISPLAY_PROPERTIES']['CONF_COLORED_BODY_BACK']['FILE_VALUE']['SRC'] ?>" data-color-body="<?= $get_model['DISPLAY_PROPERTIES']['CONF_COLORED_BODY_BACK']['DESCRIPTION'][0] ?>" data-name="color" alt="<?= $get_model['DISPLAY_PROPERTIES']['CONF_COLORED_BODY_BACK']['DESCRIPTION'][0] ?>">
										<?else:?>
					...
					<? if (!empty($get_model['DISPLAY_PROPERTIES']['CONF_COLORED_BODY_BACK']['FILE_VALUE'])) { ?>
						<div class="arrows conf-widget__arrows">
							<button class="conf-widget__arrows-prev arrows__btn"><img class="arrows__img" src="/local/templates/tinger/img/arrow-prev.svg" alt="Prev"></button>
							<button class="conf-widget__arrows-next arrows__btn"><img class="arrows__img" src="/local/templates/tinger/img/arrow-next.svg" alt="Next"></button>
						</div>
					<? } ?>
				</div>
		...
		<div class="btn__group btn__group--configurator">
			<button data-btn-specification="back-btn-2" data-simbol-code="<?= $get_model["CODE"] ?>" class="btn btn_color_green">Back</button>
			<button data-btn-specification="continue-btn-4" data-simbol-code="<?= $get_model["CODE"] ?>" class="btn btn_color_green">Continue</button>
		</div>
	</section>
<? } ?>
...
<section data-page="facilities" data-simbol-code="end_block" style="display:none;" class="conf-total container">
	...
	<div class="conf-total__visual margin_b_l">
		...
		<div class="conf-numbers">
			<button class="btn btn_color_green" type="button" onclick="openModal('default', {'crm-entity': 'lead', 'crm-title': 'Configured ATV (lead)', 'action': 'get-conf-data'})">Send request</button>
		</div>
	</div>
	...
	<div class="conf-total__modification margin_b_xl">
		<div class="conf-total__btn-container margin_b_l">
			<p class="section-subtitle margin_b_m">Your modification</p>

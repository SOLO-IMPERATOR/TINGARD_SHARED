...
<div id="equipment" class="conf-widget">
		...
		<div class="conf-widget__slider margin_b_m">
			<div class="conf-layers">
				<?foreach ($arResult["MODIFICATIONS"] as $set_img) {?>
					<img data-mods-name="<?=$set_img[0]?>" src="<?=$set_img[3]?>" alt="Image <?=$set_img[0]?>">
				<?}?>
			</div>
		</div>
	...
	<div class="conf-widget__total">
		<div class="conf-widget__total-btn-container">
			<a class="conf-widget__total-btn btn btn_color_transparent-green" href="/configurator/">Go to configurator</a>
		</div>
		<div class="conf-numbers conf-numbers_place_conf-widget">
			<div class="conf-widget__btn-container">
				<button class="btn btn_color_green" type="button" onclick="openModal('default', {'crm-entity': 'lead', 'crm-title': 'Buy <?=$arResult['PROPERTIES']['M_SHORT_NAME']['VALUE']?> (choosed modification)', 'action': 'package-conf-model-page'})">Buy</button>
			</div>
		</div>
	</div>
</div>
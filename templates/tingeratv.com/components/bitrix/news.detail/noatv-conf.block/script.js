...
$(window).load(function() {
    ...
    if ($('#equipment').length > 0) {
        $('#equipment .conf-widget__package input').first().change();
        $('#equipment .conf-widget__package input').first().click();
    }

    if ($('#engine').length > 0) {
        $('#engine .conf-widget__package input').first().change();
        $('#engine .conf-widget__package input').first().click();
    }
...
$(document).ready(function () {
     ...
     */
    $('#equipment .conf-widget__package input').change(function () {
        let selected_mod = $(this).attr("data-model");
        let model_price = Number($(this).val());
        let standart_price = Number($('#equipment .conf-widget__total .section-subtitle').attr('data-standart-price'));
        let total_price = 0;

        $('#equipment .conf-layers img').css('display', 'none');
        $('#equipment .conf-layers img[data-mods-name="' + selected_mod + '"]').css('display', 'block');

        $('#equipment .conf-widget__options tbody .table__row').css('display', 'none');
        $('#equipment .conf-widget__options tbody .table__row[data-mods-name="' + selected_mod + '"]').css('display', 'table-row');

        total_price = standart_price + model_price;

        $('#equipment .conf-widget__total .section-subtitle').text('Итого: ' + Number(total_price).toLocaleString('ru-RU') + ' руб.');
    });

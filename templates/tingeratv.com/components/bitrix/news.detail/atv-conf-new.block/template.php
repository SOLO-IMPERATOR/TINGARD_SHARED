...
<?//echo '<pre>'; print_r($arResult['PROPERTIES']['CONF_COLORED_BODY_BACK']['VALUE']); echo '</pre>';?>
					...
					<div class="conf-widget__color-list margin_b_s">
						<?for ($i = 0; $i < count($arResult['PROPERTIES']['CONF_COLOR_MODEL']['VALUE']); $i++) {?>
							...
							<label class="conf-widget__color-item">
								<input class="radio__input" data-simbol-code="<?=$arResult["CODE"]?>" type="radio" data-color="<?=$color['UF_XML_ID']?>" name="confColor" value="<?=$color['UF_NAME']?>" data-free="<?=$color['UF_IS_FREE']?>" title="<?=$color['UF_NAME']?>">
					...
					<label class="conf-widget__options-item conf-widget__options-item_type_with-img" data-modul-binding="" data-sim-code="<?=$put_dopmodules["UF_CODE"]?>" data-id-modul="<?=$put_dopmodules['UF_XML_ID']?>" data-mods-name="<?=$mods_name?>" data-name="<?=$put_dopmodules['UF_NAME']?>" data-price="<?=$put_dopmodules['UF_PRICE']?>" <?if ($put_dopmodules["UF_BLOCKED"] == 1 && $put_dopmodules["UF_CHECKED"] == 1) echo "data-default='default'";?>>
						...
						<span class="checkbox__container">
							<span class="checkbox__img-container">
								<?if(isset($put_dopmodules['UF_IMAGE'][0])):?>
								<img class="checkbox__img" src="<?=$put_dopmodules['UF_IMAGE'][0]?>" alt="<?=$put_dopmodules['UF_NAME']?>">
								<?else:?>

...
?>
<script src="https://cdnjs.cloudflare.com/ajax/libs/jszip/3.1.0/jszip.js"></script>
<?if($_GET['selectModel']):?>
<input type="hidden" id="selectModel" value="<?=$_GET['selectModel']?>">
<?if($_GET['selectModify']):?>
	<input type="hidden" id="selectModify" value="<?=$_GET['selectModify']?>">
<?endif;?>
<style>
header{
...
<section data-page="first-page" class="container margin_b_xl conf-preloader conf-preloader_active">
	...
	<div class="conf-models margin_b_m">
		<ul class="conf-models__list">
			...
			<?if($pet_productions['ID'] != 68 && $pet_productions['ID'] != 6343 && $pet_productions['ID'] != 6665):?>
...
<? foreach ($arResult['ITEMS'] as $pet_model_data) { 
	if($pet_model_data['ID'] == 68  ||$pet_model_data['ID'] == 6343  || $pet_model_data['ID'] == 6665){
		continue;
	}
	?>
	<?
...
<? foreach ($arResult['ITEMS'] as $get_model) { 
	if($get_model['ID'] == 68  ||$get_model['ID'] == 6343  || $get_model['ID'] == 6665){
		continue;
	}
	?>
	...
	<section data-page="conditions" data-simbol-code="<?= $get_model["CODE"] ?>" data-mods-name="" data-mods-price="" style="display:none;" class="container margin_b_xl">
		...
		<div class="conf-widget margin_b_m">
						...
						<div class="conf-widget__color-list margin_b_s">
							<? for ($i = 0; $i < count($get_model['DISPLAY_PROPERTIES']['CONF_COLOR_MODEL']['VALUE']); $i++) { ?>
								...
								<label class="conf-widget__color-item">
									<input class="radio__input" data-simbol-code="<?= $get_model["CODE"] ?>" type="radio" data-color="<?= $color['UF_COLOR_CODE'] ?>" name="confColor" value="<?= $color['UF_NAME'] ?>" data-free="<?= $color['UF_IS_FREE'] ?>">
			...
			<div class="conf-widget__total">
				...
				<p class="section-subtitle total-price-m" data-entity="total" data-modification-price="0" data-pack-price="" data-total-price="0">Базовая цена модели: 0</p>
...
<section data-page="facilities" data-simbol-code="end_block" style="display:none;" class="conf-total container">
	...
	<div class="conf-total__visual margin_b_l">
		...
		<div class="conf-numbers">
			...
			<p class="section-subtitle margin_b_m" id="total-price-end" style="display:none;">Итого: <span>0</span> руб.</p>
...
<script>
	function makeComment(){
		const confSelector = document.querySelector('section[data-page="conditions"]:not([data-mods-name=""])');
		let comments = `<b>Выбранная модель и модификация: ${confSelector.dataset.modsName}<br>`;
		...
		confSelector.querySelectorAll(`.conf-widget__options .conf-widget__options-item[data-mods-name="${confSelector.dataset.modsName}"] .checkbox__input:checked`).forEach(optionEl => {
		...
		comments += `${confSelector.querySelector('.total-price-m').textContent}`;
		return comments;
	}
	window.addEventListener('message', function(event) {
		if (event.data && event.data.type === 'getComment') {
...
<script src="/local/templates/tinger/js/pdf.js?v=<?=rand()?>"></script>
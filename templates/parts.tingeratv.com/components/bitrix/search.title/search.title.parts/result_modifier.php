...
foreach($arResult["CATEGORIES"] as $category_id => $arCategory) {
	foreach($arCategory["ITEMS"] as $i => $arItem) {
		if(isset($arItem["ITEM_ID"])) {
			...
			$arResultSubsections = CIBlockElement::GetElementGroups($arItem['ITEM_ID'], true);
			$n = 0;
			while ($arItemSubsections = $arResultSubsections->Fetch()) {
				...
				$arResult['CATEGORIES'][0]['ITEMS'][$i]['SECTIONS'][$n]['SECTION_NAME'] = $arItemSections['NAME'];
				$arResult['CATEGORIES'][0]['ITEMS'][$i]['SECTIONS'][$n]['SUBSECTION_NAME'] = $arItemSubsections['NAME'];
				$n++;
			}	
